...
export default function Edit({ catgory }) {
    ...
    const onSubmit = (data) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.put(`/catgory/update/${catgory.id}`, data)
            }
          })
    }

    useEffect(() => {
        setValue('cat_name', catgory.cat_name)
        setValue('cat_parents', catgory.cat_parents)
        setValue('cat_detail', catgory.cat_detail)
        setValue('cat_status', catgory.cat_status)
    }, [catgory])
    return (
        <>
                ...
                <div className="container-lg bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid gap-6 mb-6 md:grid-cols-2  p-3">
                            ...
                            <div>
                                <label for="last_name" class="block mb-2 text-sm font-medium text-gray-90">ลำดับ</label>
                                <Controller
                                    control={control}
                                    name="cat_parents"
                                    rules={{
                            ...
                            <div>
                                <label for="phone" className="block mb-2 text-sm font-medium text-gray-900 ">เปิด/ปิด</label>
                                <Controller
                                    control={control}
                                    name="cat_status"
                                    render={({ field: { onChange, onBlur, value, ref } }) => (
                        ...
                        <div className="my-8 p-3">
                            <button type="submit" className="text-white mr-3 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">แก้ไขข้อมูล</button>
